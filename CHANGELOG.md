...
### Method
- **The engine is untouched.** This is a `class-ds-tripwire.php` change only; `ds-scan-engine.php` is byte-identical to 1.9.138 and its selftest still passes 41/41. A pre-tokenise md5 skip (a real speed win) was deliberately left out: it is an engine change, and the ask was no notification spam, not speed.
