...
- **LeagueApps Table: a Beaver Builder module for schedules, fees, rosters and standings. Not released yet: on branch `feat/table-module`, being tested on ds-launchpad-7** (Alipio, 2026-09-26: "an easy way to create table using beaver UI they can also upload CSV. make sure its ajax and well sync"). Sample: the Tryout Schedule section on ds-launchpad-7's Tryouts page.
  ...
  - **Tests.** `tests/table-data-test.php` (33 assertions: storage, CSV parsing, column types and sort keys, Google links, escaping) and `tests/table-ajax-test.php` (13: import keeps no file, and who may read, store or replace a CSV; the handler before the read-only import fails 4 of them), both run with `wp eval-file`. End to end in the real builder on ds-launchpad-7: 33 builder checks and 20 front-end checks.
