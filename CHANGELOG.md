...
## [Unreleased]
### Changed
- **Theme Setting page rebuilt (performance, colour picker, UX). Not released yet: on branch `feat/theme-setting-ux`, being tested on ds-launchpad-7.** Storage is unchanged: the form posts the same 245 field names to the same handler, which writes the same Beaver Builder Global Styles (`_fl_builder_styles`), theme mods and options (a no-change save leaves every stored value byte-identical, checked against a snapshot).
  - **Performance.** The page shipped every Google font name inside each of the 10 typography groups (20,550 `<option>` tags, 1.27 MB of HTML, 128k DOM nodes) and created 47 colour-picker instances on load. Fonts now come once as data after the page renders, one colour picker is shared by every field and created on first use, and WordPress' code editor (about 1.3 MB of CodeMirror + linters) loads only when Custom Code is opened: 307 KB HTML, 8k DOM nodes, main-thread cost equal to Settings > General.
  - **Colour picker.** The swatch never showed the colour just picked (Pickr only repaints its button on save, which was switched off). Picking a global colour stored a fixed hex instead of the linked `var(--fl-global-*)`, so those fields stopped following the palette. Both fixed; linked fields show the global's name, values can be typed or pasted (hex, rgba, a global's name), and Default clears a field.
  - **Live preview.** A real page of the site beside the controls shows unsaved changes: Beaver Builder generates its global CSS from the form, and the page renders in a preview mode that holds the saved global styles in replaceable blocks (layout CSS rendered inline for that request only, cache files untouched), so clearing a value clears it in the preview too. Desktop / tablet / mobile, any page.
  - **UX.** Customizer-style layout (section rail, controls, preview) instead of nested accordions; compact typography grids with a searchable font picker that limits weights to what the font ships; segmented controls; image options shown only once an image is set; palette rows that show each variable name, carry linked fields along on rename, warn before removing a colour in use, and reorder by drag. Saving no longer reloads the page: sticky Save bar, unsaved-changes markers per section, Ctrl/Cmd+S, Discard, and a warning before leaving with unsaved changes.
  - **Security and stability audit (2026-09-27).** Fixed before release:
    - **The page could not save at all if its script failed to load** (Save rendered disabled, seven of eight sections hidden until the script ran). The page now renders with every section visible and Save enabled, and the script switches to the tabbed view once it has booted. Checked with JavaScript off and with the script blocked: every section shows, and a plain form post saves exactly what was stored.
    - **Custom CSS and JavaScript now need WordPress's `unfiltered_html`** (was: any user who can edit posts with a LeagueApps email, so a Contributor could add site-wide script). Without it the saved code is kept as it is. Also on main before this branch.
    - **Saving no longer drops Beaver Builder templates from "Featured image as banner"** (the form only lists public types, so every save removed `fl-builder-template`). Also on main before this branch.
    - **Uppercase `RGB(…)` wiped the colour** (the page accepted it, the save rejected it; in a palette row it removed that global colour and the fields linked to it). Both sides now accept it and store it lowercase.
    - **A palette name with a double space pointed linked fields at a variable that does not exist.** The page now cleans a name the way the save does before making its variable name, and shows the stored name after saving.
    - **A failed fonts request emptied the font list for the rest of the tab** (the empty result was cached). Only a real catalog is cached now; a failure is retried.
    - **Preview:** messages between the page and the preview check the sender's origin, and the preview no longer posts its address (with its nonce) to any parent; the preview page is marked uncacheable (`DONOTCACHEPAGE` and no-cache headers); a preview that never answers says so after 20 seconds instead of loading forever; no preview CSS is generated while the preview is hidden (it catches up when shown).
    - Test harness: the "clearing a saved Size" check read a hard-coded 16px; it now reads the saved value (the site's text size is 17.5px since 2026-09-27).
