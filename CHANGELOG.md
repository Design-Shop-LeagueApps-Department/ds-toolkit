...
- **What gets read (`CONTENT_EXT`).** A rule cannot fire on a file that is never opened, and three classes were invisible unless they happened to sit at depth 1, in uploads or in mu-plugins. Media extensions are now read, because the 2026-09-26 re-seed on dallaskicsfc put `w.avi` and `qjISgYFUT.m4a`, both opening `<?php`, inside real plugin trees, where the header-based polyglot rule that convicts them never got to see them. `.htaccess` is now read, because every nest in this campaign drops one with an identical fingerprint and because the web root's own `.htaccess` had been rewritten into a backdoor. `.off` is now read so camouflage cannot hide behind our own convention outside mu-plugins. Measured cost on a live install: **+13 files against the 12,608 already read out of 22,097**, and a before/after run of both engines over that whole site produced **zero new findings**. Archives are deliberately excluded: a real `.zip` is binary so its PHP is compressed and unreadable to the polyglot test, while "every plugin zip contains PHP" is an established false positive that has already cost two allow-list hashes.
- **The doubled-directory nest `.htaccess`, as a rule.** Matched structurally (deny every PHP casing, then re-allow exactly `index.php` and `cache.php`) rather than by hash, because two md5 variants of byte-identical content exist in a single incident, differing only in trailing whitespace; the known md5 is kept as a backstop. Sweeping this fingerprint by hand found 11 nests on shoreshots where the alert named 1, 18 on dallaskicsfc where it named none, and 4 more buried in genuine `wordpress-importer` and `defender-security` vendor trees. Verified against the real files: all six nest `.htaccess` copies on dallaskicsfc now score CRITICAL, and the deny-only variant stays silent because it is indistinguishable from real hardening.
- **A web-root `.htaccess` that allow-lists executable filenames.** dallaskicsfc's `/www/.htaccess` denied every casing of `php` and then permitted 108 filenames beside the genuine WordPress entry points; with every shell removed it was still a loaded gun, and the scanner had been reading that file all along with no rule to look at it. The rule keys on **filenames that are not present on disk**, not on list length: a legitimate hardening `.htaccess` in exactly this shape permits files that exist, and the first draft convicted on length alone and printed `admin-ajax.php` as suspicious, which is how a rule earns being ignored. On the real file it reports `108 executable filenames, and 30 of them are not present on disk (adminfuns.php, chtmlfuns.php, cjfuns.php, ...)`, which is precisely the attacker's appended set.
- **Creating an administrator with no capability or credential check**, scored on its own. `wp_create_user` was **never matched at all** (only `wp_insert_user` was watched), so `wp-amdin.php` on surfsidevolleyball, which creates an admin called `kralkenan` in plain unobfuscated PHP, scored nothing. A registration flow or setup wizard carries `current_user_can` or a nonce and stands the rule down.
- **`goto` flattening on a WordPress load path**, scored on its own. The existing rule needs eval or a decoder alongside the `goto`, and the north-shore-stars `wp-content/db.php` (74 bytes, `goto`-flattened, loading `zip://index.zip#index`) had neither, so it scored below CRITICAL on 2 of the 3 sites carrying it. On `db.php`, `object-cache.php`, `advanced-cache.php` or anything in mu-plugins, the obfuscation alone is the finding; a `goto` state machine in ordinary plugin code is untouched.
### Fixed
- **Our own disabled mu-plugins no longer email CRITICAL.** `.php.off` is our documented way to disable a mu-plugin and no web server executes it, so, exactly like the `.html` template case, position alone cannot convict: it now scores the same de-weighted REVIEW, or stands down entirely when a plugin header is present. The behaviour engine still judges the file, so a `.off` that really holds a shell is still convicted. Calibrated against the two real files on marylandjr: the first draft required a plugin header and the file that actually emailed (`ds-cdn-ttl-test.php.off`, 295 b) does not have one, so that draft did not fix the reported case at all. Verified on the real files: CRITICAL becomes REVIEW and NONE respectively.
- **A leftover `.maintenance` is recognised instead of hashed.** Its whole body is the `$upgrading` epoch, so every instance has a different md5 and the allow list can never win; two hashes in the list cover exactly two files. A `.maintenance` carrying anything else still falls through and is judged.
- **`MU_MANAGED` covers the fleet-recurring helper**, and its cap moves from 4096 to 8192 bytes so `speed-kit/ds-cache-purge-globals.php` (4854 b) fits. SPEED.md makes that file the standard slow-site fix, so without this a rollout emails CRITICAL once **per site**. The gate is still name plus size plus a no-obfuscation check and deliberately not the bare `ds-` prefix, because the Sep 2026 attacker overwrote `ds-origin-guard.php` with an 8 KB loader.
