...
## [Unreleased]
### Added
- **Post Loop: Manage entries, edit what a loop shows without the dashboard. Not released yet: on branch `feat/loop-manager`, tested on ds-launchpad-7** (Alipio, 2026-09-27: "click edit staff and more option will show, you can also order the staff in the list, any edits will sync to the staff post type"; "yes only when its publish"; "we can use this also for commitments, alumni"). A new section under Layout in the Post Loop settings lists every entry the loop's query covers (its post type and category filter), shown or hidden, in the loop's order. Each row opens a form built from the post type's ACF fields (text, email, URL, number, textarea, rich text, image, select, true/false, date; anything else links to the dashboard), plus name, photo, a simple rich-text bio (bold, italic, link, list), categories and a Show on the site switch. Entries can be added (a new one starts in the loop's category, so a new athlete on Commitments lands in Commitments), removed (to the Trash, restorable until you publish, never deleted) and reordered by dragging or with Move up / Move down.
  - **Nothing is written until the page is published.** Changes are held in the module (one hidden setting, `dsm1:` + base64 JSON) and written to the posts on Publish (`fl_builder_after_save_layout` with `$publish`), then cleared; Cancel or discarding the draft throws them away. The builder preview shows them straight away by overlaying the pending values on the loop's query and on the title, meta and featured-image reads while the module renders. Only fields that were actually changed are written, so an edit made in the dashboard meanwhile is kept.
  - Every write checks the user's rights for that post (edit, publish, delete, create); rich text keeps only p, br, strong, em, links and lists, with script and style blocks removed; select values must be one of the field's choices; images must be Media Library images. Order is written as `menu_order`, numbered so the loop shows the list top-down in whichever direction it sorts. Loops that share a post type share its order (Staff and Coaches; Commitments and Alumni).
  - **Security and stability audit (2026-09-27).** Fixed before release:
    - **Publishing stripped backslashes from every module on the page.** The change set was cleared with `update_post_meta()`, which unslashes, so CSS like `content:"\201C"` and a `\d+` regex in an HTML module lost their backslashes in the live and draft layout. It now goes through Beaver Builder's own writer, which slashes first. Proven with a control: the old write turns `\201C` into `201C`, the new one keeps it.
    - **The panel could run script from a crafted change set** (stored XSS in the builder). The `dsm1:` base64 envelope passes Beaver Builder's kses check, and a Trash ID or rich-text value went into the panel as HTML. The server now re-cleans the change set when the module is saved (only numeric or `n<k>` keys, integer IDs, known statuses, text through the same cleaners as publishing, image URLs rebuilt from their IDs), the panel re-checks what it loads, looks rows up without building selectors from keys, and puts rich text into the editor through an allowlist. Proven in the builder: a planted payload ran with the old script and does nothing now.
    - **A change set is applied once.** Its hash is recorded before anything is written, so a restored revision, a History undo, a duplicated module, a global module's template or a retry after a timeout cannot replay it (re-trashing restored entries, creating new entries twice). It runs before Beaver Builder takes its revision, the publish's revisions are scrubbed, and a global module's template is cleared too.
    - **Changes are held to the rights of the person who made them** as well as the publisher's: the save stamps the author, so an admin publishing a page no longer carries out edits a lower role saved there.
    - **Rich text keeps its markup.** Saving now allows post markup (tables, headings, images) minus scripts, and text the simple box cannot hold (a Teams schedule table) is shown read-only with a link to the dashboard instead of being flattened. Names keep the inline markup the cards render (`<br>`, `<strong>`).
    - **Dragging only renumbers menu_order when the loop sorts by it**; other loops and Nested Pages of the same type are left alone. Terms beyond the 200 the panel lists are kept. The change set must be for the loop's own post type.
    - **Preview:** it only adds entries of the loop's type that the editor may edit (it could show any post by ID), cleans titles and fields like a publish, uses random preview IDs cached for a minute at most, and handles a loop inside another loop's template. An empty Include list no longer hid entries being shown again (found by the new tests).
    - **Smaller fixes:** Remove then Restore kept the entry in the list; the list follows the loop's Include / Exclude picks; numbers reject infinity; dates must be Ymd; multi-select fields link to the dashboard; the page is purged again after the entries change; the list request primes term and thumbnail caches.
  - Tests: `tests/loop-manager-test.php` (52 assertions: storage, per-type sanitizing, filters, payload cleaning, the author's rights, applied once, a duplicated module, backslashes kept, order only for menu_order loops, unlisted terms kept, preview limits, and that nothing is written without rights, for another post type or on a draft save; the write tests use temporary posts and delete them). End to end in the real builder on ds-launchpad-7 against a snapshot of the sample posts, restored after: 30 checks for Staff (preview before publish, Cancel discards, rename, fields, hide, Move up, drag, add with bio and photo, publish, order, remove / restore / Trash) and 10 for Commitments (category filter, image and select fields, new athlete in the page's category).
...
- **The bundled `includes/known-good.md5` was 66 hashes behind the generator.** It was last regenerated for 1.9.141 (#210), and every vendor file cleared since then was added to `fleet-audit/bin/gen-known-good.sh` without regenerating the file the plugin ships, so those suppressions existed only in the remote list. Sites whose remote fetch fails were missing all 66. Regenerated: 16,176 to 16,247 entries.
### Added
- Four hashes from the 2026-09-25 19:05 to 2026-09-26 12:29 UTC alert wave, three of its four sites being false positives: the two WebToffee importers that alerted together on surfsidevolleyball.com (`order-import-export-for-woocommerce` 2.7.8 and `product-import-export-for-woo` 2.6.7 ship the same `admin/modules/import/import.php`, whose `eval` is the plugin's own mapping-expression evaluator, both byte-identical to wordpress.org), our own `ds-banner-featured-wins.php` on mdjrs.org (the site-specific precursor to the 1.9.159 banner fix; hooks only `pre_post_update` and `acf/save_post`, no dangerous markers, no file writes, no network), and `speed-kit/ds-cache-purge-globals.php`, added pre-emptively because SPEED.md makes it the standard slow-site fix so it lands on site after site and trips the mu-plugins rule every time.
...
## [1.9.160] - 2026-09-25
### Added
- **Tripwire: four backdoor families the fleet copy of the scan engine could not see.** The engine is maintained in two places, the plugin copy that actually runs on every site and a working copy in the fleet-audit workspace, and the rules written after the north-shore-stars incident (2026-09-21) went into the working copy only. The plugin copy therefore shipped without them for four days: running the 50-assertion selftest against it fails 4 (`DSSCAN_ENGINE=<plugin>/includes/ds-scan-engine.php bash fleet-audit/bin/ds-scan-selftest.sh` reported `46 passed, 4 failed`), and the failures are the ZISI polymorphic backdoor that sat in an inactive fake plugin through three consecutive cleanups of that site, and the 74-byte `wp-content/db.php` drop-in that loads its payload from `zip://index.zip#index`. Ported verbatim, so the plugin copy is now byte-identical to the audited one and the selftest passes 50 of 50 against both. Two new rules: `wrapinc`, an `include`/`require` of an archive or data stream wrapper (`zip://`, `phar://`, `compress.*://`, `data:`) or of a decoder call, which is how a loader keeps its code out of any `.php` file on disk; and `wrapasm`, the same wrapper assembled from split string literals (`$cron = ["zi","p:/","/index.zip#index"]; include implode("", $cron);` scored nothing before). Plus three content markers for the ZISI banner and its decoder-proxy class, whose folder and plugin names change per site. `wrapasm` only fires when the file also includes something, so a vendor file that merely compares a path against `phar://` is not flagged; that false positive was calibrated on north-shore-stars, where an earlier form of the rule was about to quarantine two legitimate vendor files.

## [1.9.159] - 2026-09-25
### Fixed
- **Pages: the Featured Image could not be changed or removed when a Banner Background Photo was set.** The page banner sync always let the Background Photo win, so on Update it copied the banner photo back over whatever Featured Image the partner had just picked, and a removed Featured Image came straight back (Alipio on ds-launchpad-7, 2026-09-25: "it cant be change it just revert to the set features. even removing it and click update it doesnt show"). The sync now follows whichever of the two the partner actually changed, including a removal: a changed Background Photo still updates the Featured Image, and a changed or removed Featured Image now updates or clears the Background Photo. The classic editor saves a featured-image pick over AJAX before the Update click, so the change is recorded when `_thumbnail_id` is written (one post meta flag, `_ds_banner_featured_touched`, cleared on the next save) rather than guessed at save time. A page whose banner photo was set without a Featured Image still gets it filled in on save, as before. `tests/page-banner-sync-test.php` (run with `wp eval-file`) drives the editor save path through 11 cases; the previous build fails 4 of them.

## [1.9.158] - 2026-09-25
### Added
- **LeagueApps Programs: a fetch ledger on the LeagueApps settings tab** (Alipio, 2026-09-25: "so that it wont overload the api call if this will be used on thousands of sites"). The traffic itself was already bounded: one fetch per ten minutes per site with any traffic at all, none without, which is exactly the `Cache-Control: max-age=600` LeagueApps sends with the feed, plus a refetch lock, a failure back-off and no retry into a 429 or 5xx (1.9.154). What did not exist was any way to see the number a given site actually sends, and on a host whose object cache drops the transient between visits that number can be far higher than the cache implies with nothing to show it. The tab now shows the last 24 hours (fetches, HTTP requests including retries, failures) against what the cache allows for the configured sites, flags a site that is over that budget by more than a quarter, and lists the last twelve fetches with when, site, result, requests, duration, program count and reason (first load, cache expired, manual refresh). Stored as one non-autoloaded option (the last 50 fetches plus per-hour counters for a rolling day), not a file: a file in the plugin folder is flagged by the fleet integrity scanner, wiped by a plugin update, and unreadable from wp-admin. A `ds_programs_fetch` action fires with every entry for a site that wants its own log. No change to what is fetched, when, or how it renders; `tests/programs-feed-test.php` grows from 42 to 63 assertions, including that the ledger's request count equals every HTTP call the stubbed transport received.
...
- **Review hardening (1.9.154):** a request that finds the refetch lock held serves the stale copy at once instead of sleeping up to 1.5 s on a PHP worker (only a first-ever load with nothing cached waits); the lock is re-armed before each site so a slow multi-site fetch cannot outlive it; the admin-bar Refresh permits one refetch a minute site-wide.
...
- **Credentials live once per site, on a new Settings > DS Toolkit > LeagueApps tab** (site ID + public widget key, several rows for a partner with more than one LeagueApps site), never inside a module instance and never in a page layout. The feed is fetched server-side with three retries, cached ten minutes, and the last good copy is kept seven days, so one dropped mobile request can no longer blank the table with "Invalid Site Id or Api Key" (the hosted widget's `Promise.all` bug). One site failing no longer takes the others down. Editors get a "Refresh LeagueApps listings" admin-bar link; saving the tab clears the cache.
- **Everything the table can show or filter on comes from one field catalogue**, twenty-two values: program, age group, dates, start, end, month, sport, gender, type, youth/adult, registration status, season status, days, location, city, season, level, format, sponsor, price, spots, register. Columns and filters are drag-ordered repeaters that pick from it; each column has its own header text, alignment, width and wrapping. Month options sort chronologically, age groups numerically (`9u` before `10u`, and soccer's `U9` before `U10`), days in week order, and the Days filter matches a row that contains the day. The API has no server-side filtering (verified: `type=`, `sport=`, `mode=`, `limit=` all return the full 484 KB), so scoping is done here from the cached copy.
- **Dates and months are derived from the program's real start and end**, not from text an admin typed into an unrelated field. On the reference site 24 of 169 tournament date ranges and 10 of 219 months disagreed with the program's own dates (typos like `10/14/26`, "April" on a June program). A partner whose admins do type meaning into Sponsor / Season / Experience Level keeps that through the Field mapping section, which reads any catalogue value from a raw LeagueApps field instead. Registration status is derived where LeagueApps leaves it blank (sold out from spots, closed from the registration end date).
- **A Style tab that covers the whole table**, with four palette-neutral presets (striped, bordered grid, minimal, card) as a base layer that any field overrides: table background, border mode (row lines / full grid / outer frame), width and colour, corner radius, shadow; header background, colour, typography and its own underline; row background, stripe, hover, colour, typography, program-name emphasis and responsive cell padding; the filter bar's label colour and typography plus the dropdown's fill, text, border, radius and width (the hosted-widget port hardcoded a white dropdown, which broke on every dark section), gap and spacing; the register button inheriting the site Button or fully custom including typography and padding, and three sold-out treatments; the empty-state text; and the phone card view (card background, padding, radius, gap, bottom line, field-label colour, size and case, program-name size), which now breaks at the site's own Beaver Builder small-screen breakpoint instead of a hardcoded 767px. Program-name emphasis is emitted with the column class so a row typography weight no longer flattens it.
- Verified on a stub render harness against the reference site's real 226-program feed (38 assertions: row counts computed independently from the payload, no key in the HTML, escaping, `//` register links normalised to https, ordering, mapping, every Style field's CSS) and in a real browser (combined filters, multi-day match, clear, counts, no sideways scroll at 390px, header hidden and name first on phone). Desktop and phone screenshots of four presets reviewed.

## [1.9.142] - 2026-09-20
> Ships as 1.9.142: v1.9.141 was published in parallel (#210, the Duplicator Pro clearance below) while this was in review.
### Fixed
- **Ported from `fleet-audit/bin/ds-scan-engine.php`, where it existed uncommitted: a closure literal passed to a callback sink is not a dynamic call.** New `dsscan_range_is_closure_literal()`. `array_map(function($file){...}, $data)` has its callback fixed at compile time, so no request value can redirect it. The taint set is keyed by variable NAME for the whole file, so a closure PARAMETER reusing a name tainted in an unrelated method (Everest Forms 3.6.0: `$file` from `$_POST` at line 136, reused as a closure param at line 1408) scored stock wordpress.org-verified code **CRIT 125** and emailed the fleet (juventusny.com, twice). Only the START of the range counts, so `array_map($_GET['f'], $x)` still fires; the selftest carries both assertions, **41 -> 43**. This fixes the class at the rule, where the 1.9.140 hash only covered one version. The two engine copies are byte-identical again.
### Added
- **False positives no longer need a release. Tripwire now fetches a fleet-wide allow list and deny list from this repository once per cron cycle.** Every vendor false positive so far (Forminator, GTranslate twice, PixelYourSite twice, Duplicator, Gravity Forms, Everest Forms, wp-console) cost a release and a multi-hour push across ~1,100 sites to silence one file. From this release a verified hash committed to `lists/tripwire-allow.md5` is live on every site within the hour. Same for `lists/tripwire-deny.md5`, which NAMES malware in the alert (`KNOWN MALWARE: <name>`), so a new payload hash is recognised fleet-wide without shipping code. Alipio: *"lets prevent spamming notif the worse bug double check before fleet releasing."*
- **Designed so the worst it can do is nothing.** The remote lists can only ADD to the bundled `includes/known-good.md5`, never remove. A deny entry only labels a finding the engine already scored on behaviour; it never creates one, so a wrong deny hash cannot manufacture an alert on a clean file. Every fetch failure leaves the bundle in charge, which is the exact behaviour of every earlier release: not 200, not `text/*`, over 512 KB, a transport error, anything thrown. **A body with zero valid hashes is a failed fetch, never an empty list**, because an HTML error page, a rate-limit notice and a truncated response all parse to zero. A failure after an earlier good fetch serves the **last good copy** (30-day cache). Failures retry every 15 minutes; successes cache one cycle plus a per-site jitter (`crc32(home_url) % 900`) so the fleet does not hit GitHub in the same second. The fetch runs **before** the scan deadline is stamped, so a cold 4-second fetch never eats scan budget. Neither loader can throw; the call site catches anyway and falls back to "suppress nothing", the loud direction.
- **Public on purpose.** The lists hold md5s and nothing secret. A private repo would put a read token on every one of ~1,100 sites, where the first compromised site leaks it. The security property is WRITE access to `main`. (Branch protection on `main` is currently off and four accounts can push; recommended, and a decision for Alipio.)
- Per-site fetch health is recorded in state (`content.remote`: `fetched` / `cached` / `fail (...)`, with counts), so a scan that suppressed or named more than the bundle can say where that came from, and a dead endpoint is visible rather than silent.
- Reachability verified before building: a Flywheel container and a WP Engine install both fetched `raw.githubusercontent.com` from PHP in ~200 ms.

### Method
- **Beyond the ported closure-literal fix, the engine is unchanged.** The remote lists live entirely in `class-ds-tripwire.php`; `ds-scan-engine.php` differs from 1.9.138 only by that one function and its selftest passes **43/43** against the shipped copy. A pre-tokenise md5 skip (a real speed win) was deliberately left out: it is a bigger engine change, and the ask was no notification spam, not speed.
- The parser is a pure function so the unit test can feed it garbage. `fleet-audit/bin/ds-tripwire-unit.php` goes from 21 to **53 assertions**: HTML error pages, rate-limit notices, empty and non-string bodies, 31- and 33-character near-hashes, truncated last lines, CRLF, uppercase, named entries; and for the fetcher, timeout, 404, 200-with-HTML, wrong content-type, oversized, a good fetch cached with jitter, a cache hit making no HTTP call, and an outage after a good fetch serving the last good copy.
- Seed lists ship with today's eight verified allow hashes and three named deny hashes (the mightykicks `cache.php` payload, the fsafc `.mp4` stager, the northshorestars `db.php` cloaker), so the first fleet fetch is a clean `fetched` rather than a `no valid hashes` failure.
- `lists/README.md` documents how to add a hash and every failure mode.

## [1.9.141] - 2026-09-20
### Fixed
- **Duplicator Pro's `SnapUtil.php` is not a web shell** (#210, released without a changelog entry; recorded here so the history has no hole). `duplicator-pro` 4.5.22.5 `src/Libs/Snap/SnapUtil.php` (`0c9e96ab…`) fired the `cbinput` rule twice, exactly like the free build cleared in 1.9.140. Premium, so hand-read rather than wordpress.org-matched: at lines 217, 235, 823 and 875 the `$func`/`$callback` values are parameters of Duplicator's own `arrayGroupBy` / `arrayMapRecursive` / `arrayFilter` helpers (235 is an array callable to its own class), and the file's only superglobal reads (478-503) are a separate by-name accessor with no data flow into those callbacks. Note the Pro file is NOT the free file with a different header: 269 code lines differ (return types, a new `getCallbackName`), so the free hash never covered it. Site: bigtimehoops.net.
...
- **The fix is the rule's own argument, applied consistently.** Three lines above, the mid-file case already reasons: *"nginx never hands .js to PHP-FPM, so it cannot execute - report, do not cry wolf."* That holds identically for `.html`: no web server hands it to PHP-FPM, so such a file cannot execute on its own request and is only reachable through an `include()`. `.html`/`.htm` now score **45** (REVIEW: recorded in state for `fw-check-site-v2.sh`, never emailed) and need a second signal to convict. **Every other extension keeps the full 120**, because an image, a media file or a dotfile is never a legitimate PHP template - which is what still convicts the akismet husk dropper (61 KB of pure PHP named `logo-*.png`) and yesterday's real find, a `goto`/`metaphone` polyglot named `mrLasYweNhUTjSB.m3u8`.
- Verified against the real files, not a fixture: the same 289 `.html` files on that install score **242 CRIT / 47 REVIEW** on the shipped 1.9.137 engine and **0 CRIT / 289 REVIEW** on this one.

### Method
Two assertions added to `fleet-audit/bin/ds-scan-selftest.sh` (now **41**, all passing against both engine copies): a Forminator-shaped `.html` template must **not** be CRIT, and a `.html` holding `system($_GET[...])` plus `eval(base64_decode($_POST[...]))` still **must** be. Both were shown to fail against the pre-fix engine before being accepted, so the tuning cannot be quietly undone. `fleet-audit/bin/ds-scan-engine.php` and `includes/ds-scan-engine.php` are byte-identical again; the CLI copy is what `fw-clean-site.sh` and `fw-check-site-v2.sh` use, and it had the same bug, which is why a cleanup plan on palsports.org proposed quarantining 30 Forminator templates.

### Still open, deliberately not in this release
The kit `.htaccess` md5 `17390d4bee89c5561d4db280997b9699` (byte-identical on three infected sites, 2026-09-19) is not yet a rule, and the `cbinput` stacking false positive on Divi's `__call` dispatch is unchanged. Both are new detection logic; adding them immediately before a 500-install rollout would trade a known false-positive flood for an unmeasured one.

## [1.9.137] - 2026-09-19
### Fixed
- **The known-good list now covers the vendor code the fleet actually runs, which is what stops stock plugins emailing as web shells.** 1.9.136 shipped the hash gate and it worked: on a 20-site pilot it cleared **235 of 288** flagged files by itself. The list was just short. **5 of those 20 sites (25%) still sent 21 CRITICAL emails, every one of them stock vendor code** - Gravity Forms, GTranslate, UpdraftPlus and 14 Forminator template files. Extrapolated across the ~660 Flywheel sites still on an older build, that is roughly 130 sites and ~550 emails, which is precisely the "alert nobody reads" failure the content layer was built to avoid. `includes/known-good.md5` goes from 16,085 to **16,144** hashes: **48** vendor files from the pilot plus the **11** files of our own that 1.9.133-1.9.136 changed.
- **The list had stopped covering our own plugin.** It was generated at 10:41 UTC on 2026-09-18, before the 1.9.136 manifest rows existed, so the 11 files that actually changed across 1.9.133-1.9.136 were absent - including `includes/ds-scan-engine.php` and `features/class-ds-tripwire.php`. Those two survive only on the absolute-path self-exemption, so the hash-gate backstop behind that exemption did not exist. Regenerating from the current manifest restores it. `known-good.md5` can never contain its own hash, and a release always changes files the shipped manifest predates, so the self-exemption stays load-bearing by design.

### Method
Every hash added carries evidence, recorded per line in `fleet-audit/bin/gen-known-good.sh`. The engine was run in **CLI mode** on each pilot site (read-only: no email, no option write, no DB touch), then **each plugin was verified individually** with `wp plugin verify-checksums <plugin>`. 13 plugins returned "Verified 1 of 1", so those files provably match wordpress.org. 6 premium plugins have no wordpress.org checksums, so each flagged file was **read on a live container** instead: Gravity Forms' `create_function()` is stock 2017 code with the upload gated by `check_admin_referer()`; Forminator's `.html` files are PHP-generated CSS partials, since WPMU DEV uses that extension for PHP templates, and its `upload.php` guards correctly with `wp_generate_password` + `sanitize_file_name` + `wp_check_filetype` + `wp_unique_filename`; Beaver Builder calls back into its own registered field config behind `is_callable`; WP Engine's migration plugin writes to `wp_tempnam()`; Smart Slider's controller opens with `validateToken()` and its `range()` builds a random filename suffix, not a function name; wp-defender's is a `#!/usr/bin/env php` vendor CLI tool.

**Two findings were deliberately NOT cleared.** `wp-file-manager` stays out, as before. New: **`css-hero/image_upload.php`** stays out. It builds its destination path straight from `$_FILES['newpicture']['name']` with no `sanitize_file_name()`, and its only gate is `mime_content_type()` against `get_allowed_mime_types()`, which never compares the extension. It returns 500 standalone because `wp_upload_dir()` is undefined without WordPress, so it is not directly exploitable, but the finding is legitimate and must keep firing. A false positive is a file we have read and understood; an unread file is not a false positive.

## [1.9.136] - 2026-09-18
### Added
- **The `lxml.ahkj.lol` remote loader is now named, not just scored.** Found 2026-09-18 byte-identical on plljuniors.com and summitfieldhockey.com as a 567-byte `/www/.gitignore` **polyglot** (md5 `79880f0eb3d9cddb3198626bcbd6c081`): it opens with a PHP tag despite the filename, assembles its variable name with `chr(0x78^0x1d)`, and curls its payload from `http://lxml.ahkj.lol/gitignore.txt` at runtime. The URL is **base64, never plaintext**, so the fingerprint matches what is actually on disk: `aHR0cDovL2x4bWwuYWhrai5s`, the 24-character prefix that encodes `http://lxml.ahkj.l` and stays stable whatever path follows, because base64 encodes in fixed 3-byte groups. A plaintext `ahkj.lol` variant is listed too. The engine already caught this behaviourally as a polyglot; the fingerprint means the alert says which campaign it is.
- **`KNOWN_BAD_MD5`: malware identified by exact hash gets named in the alert.** Hash, not filename - this dropper wears a `.gitignore` name precisely because a filename is worthless as evidence. Findings whose md5 matches lead with `KNOWN MALWARE: <name>`, which matters when someone is triaging twenty alerts at 3am.

## [1.9.135] - 2026-09-18
### Added
- **Tripwire can now clear known-good code, which is what stops it reporting stock plugins as web shells.** The engine's own header has always said "WHAT MAKES IT SAFE TO BE AGGRESSIVE: the CALLER suppresses findings whose md5 is known-good" - and `fw-check-site-v2.sh` does exactly that with the fleet manifest. This plugin had no gate at all, and the difference was stark: on thepaohio.com the scanner reported **3** findings and the in-plugin scan alerted on **100 files**. Same engine, same rules. `includes/known-good.md5` now ships 16,085 hashes (the blueprint stack plus five hand-verified premium files) and a finding whose md5 is on the list is dropped. Suppression is by **hash, never by path or name**: a file cannot be made to collide with a verified hash, and if it matches then it is that file. A missing or unreadable list means no suppression, so the failure mode is "keep scanning", never "go quiet", and the count of suppressed findings is recorded in state as `content.cleared` because a scanner that goes quiet has to be able to say why. Regenerate on each release with `fleet-audit/bin/gen-known-good.sh`; a stale list only ever costs a false positive.
- **Our own Search Console verification files are no longer reported as attacker tokens.** A `google<16hex>.html` in a web root is indistinguishable from an attacker's by shape, so the five legitimate files placed on 2026-09-18 would each have been flagged. New `KNOWN_GOOD_GSC` list, checked before the attacker list.

### Fixed
- **A callback name built from request input joined to a literal is not an arbitrary function call.** The rule's own comment said "only when it is a bare value" but it never tested for bareness, so it fired on any dynamic callback. Beaver Builder core does, in `classes/class-fl-builder-auto-suggest.php`: `if ( str_starts_with( $action, 'fl_as_' ) && function_exists( $action . '_value' ) ) { call_user_func_array( $action . '_value', ... ); }` - guarded three ways, with the callable space limited to the `fl_as_*_value` functions that already exist. That scored **HIGH 60 on every one of ~1,200 fleet sites**. An affixed name (literal of 3+ chars) now scores 25 with honest wording; a bare one still scores 60.
- **`call_user_func($_POST['f'], $x)` scored NOTHING, and now scores HIGH 60.** This is the textbook dynamic-call backdoor and it was invisible: `$_POST['f']` contains a `[`, and the method-dispatch stand-down treated **any** `[` as an array callable. `[` is overloaded in PHP - a callable literal `[$o,'m']` never follows a variable, an index always does. Found by writing a test that asserted the bare case still fires after the change above; it did not. **This was a detection gap, not noise.**
- **`preg_replace()` with the `/e` modifier is scored by whether this PHP can execute it.** PHP 7 removed `/e`: the call warns and returns null, so the replacement never runs. Scoring it as live RCE made stock Headway theme code **CRITICAL 132** on three fleet sites, and the flagged line is the documented WordPress serialization-repair idiom which the file's own comment describes. Full weight (120) stays where it can execute (PHP 5); on PHP 7+ it scores 15 as a legacy code smell. The self-test assertion is now version-aware and still demands CRITICAL on PHP 5, which is what proves the gate is wired the right way round.
- Three more attacker Search Console tokens added to `KNOWN_BAD_GSC`, taking it from 8 to 11: `517c3ca44ca38f1d` (found on **two** unrelated sites, the same many-sites-one-token pattern that identified the previous two), `48d14fbc900051df` and `3222669cc1aaedd0`. All three had scored only REVIEW because they were unknown.

## [1.9.134] - 2026-09-18
### Fixed
- **Tripwire no longer reports itself as a web shell.** Since WordPress 6.3 the plugin updater keeps a rollback copy of the plugin it is replacing at `wp-content/upgrade-temp-backup/plugins/<slug>/`. The content scan walked it, and because the engine's self-exemption is by **absolute path** only the live copy was exempt, so our own files were tokenised and scored on the campaign markers they exist to hunt: 915 on `includes/ds-scan-engine.php` (it carries `WDG-CORE`, `BypassServ` x4 and `flf_pnpur_ybt` as literals) and 140 on `features/class-ds-tripwire.php`. Both are CRITICAL, and CRITICAL emails. Observed on somerssports.org at 17:15 UTC and **five times in one evening** on oklahomabasketballacademy.com, whose gated re-scan then showed no malware whatsoever. Left alone it would have fired on every fleet site after every toolkit update. `wp-content/upgrade-temp-backup/` and `wp-content/upgrade/` are now skipped: WordPress never executes either, so neither is a load path. The self-exemption is deliberately still absolute-path, because content-based self-recognition is spoofable.
- **"Executable" now requires room for code.** A file counted as executable on its extension alone, so skyy2win.com's **47,082 identical 7-byte `<?php` directory-index guards** (md5 `3beefb00777a6bd04265b7d33c23efa9`) each counted as one. That site produced 2,494 HIGH plus 6,326 REVIEW findings and a **7.6 MB alert email that Gmail delivered as an attachment nobody could open**, while **242 real payloads sat in the same tree unread** - a doorway kit, fake `wp-login.php` and `wp-cron.php`, and an `accesson.php` backdoor. Files of 16 bytes or fewer are no longer executable content; the smallest useful shell, `<?php eval($_GET[0]);`, is 21. The **name**-based shell rules are deliberately not gated on this, so somerssports.org's 0-byte `filefuns.php` is still caught by name - which is how it was caught.
- **A method named like a builtin is not the builtin.** `call_user_func_array( array( $this->redis, 'eval' ), $args )` was read as PHP's `eval`, because the callback literal was matched anywhere in the argument range. It is the Redis client's `eval` **method** (Redis runs Lua server-side), and WPMU DEV Hummingbird's Redis object cache calls it on every cache read: stock 3.21.2 scored CRITICAL 110 on 1812sports.com and the cleanup quarantined a vendor file. The enclosing bracket is the only thing that separates the two cases, since `usort($a,'system')` and `array($o,'system')` are the same token shape locally, so the rule now walks back to the bracket enclosing the literal and stands down only for an array callable. A bare string callable in any argument position still scores.
- **A site's own bespoke web-root code can be declared, per site.** New filter `ds_tripwire_root_extra_allow` takes an array of **exact** folder or file names (never a regex, so a site cannot break the scan with a bad pattern) and is applied on top of the built-in allowlist. brsoccer.org keeps a 2019 schedules application in `/www/events/`, `/www/classes/` and `/www/require/` that `bb-theme-child/header.php` requires on every page; quarantining it took that site down for ~25 minutes. Worse, `/events` is also a published page slug, so the page-shadow rule scores it CRITICAL, and CRITICAL deliberately never baselines, so it alerted every single day with nothing anyone could do about it. The names now live in a small mu-plugin on that site rather than in this plugin's hardcoded list, because they are that site's facts, not the fleet's.
- Correction to the 1.9.132 notes: the per-site kill switch is `wp option patch insert ds_toolkit_settings tripwire_content_enabled 0`. `patch update` fails with `No data exists for key` when the setting has never been set, which is the normal case since it defaults to on by absence. Use `patch update` only to change it back to `1`.

## [1.9.133] - 2026-09-18
### Added
- **Gradient headline text `{g}…{/g}` in the Heading and Hero Banner modules (GH #200).** `Train harder. {g}Compete brighter.{/g}` renders the wrapped text with a left-to-right two-colour gradient (CSS `background-clip: text`), and only that text: the rest of the headline keeps its normal colour, and the eyebrow, subtext, stats, buttons and overlay are untouched. Two new colour fields under Style → Colours, **Gradient Start Colour** and **Gradient End Colour**, with the same picker and global-colour connections as the accent fields. A blank value falls back to the module's accent colour (Heading Accent / Accent Word), and when that is blank too, to the global accent. The gradient follows the text across line breaks and responsive wrapping (one continuous sweep, not restarted per line), verified in the browser at desktop and phone widths.
- **Safe by construction.** The marker is swapped after the existing inline sanitiser, so it adds a fixed `<span>` and nothing else; text between the markers has already been cleaned. Only a matched `{g}…{/g}` pair is converted, so a stray `{g}` or `{/g}` stays as literal text instead of leaving an unclosed tag. Markers are not nestable: wrap separate ranges rather than putting `{g}` inside `{a}` or `{outline}`. `{a}` and `{outline}` are unchanged.
- **Byte-identical output for everything that does not use it.** Rendered against the previous release on a stub harness: headings and heroes without `{g}` produce the same HTML and the same dynamic CSS as before, including modules with an accent colour set, because the gradient variables are only emitted when a gradient colour is set or the headline contains the marker. Style 2 (page banner) headlines come from the page title or the ACF lead heading; set the two gradient colours on the module to style a `{g}` placed there.

## [1.9.132] - 2026-09-17
### Added
- **Tripwire scans every file by BEHAVIOUR, hourly.** Until now every check asked "does a name match?". On dallaskicsfc.com, of the five malicious files inside one fake plugin, four were caught only by the `Nx###.php` filename rule and the fifth (`dsa.php`, an `eval(base64_decode(strrev(curl())))` loader) by no rule at all; the fake plugin carried a valid `Plugin Name:` header so the folder check waved it through. Rename the shells and the site read clean. And `w5x9k2m7q4.php` on the same site spelled `"e"."x"."e"."c"` from fragments precisely so that grep finds no `exec(`. The new `includes/ds-scan-engine.php` tokenises each candidate file with PHP's own `token_get_all()` and scores what it does: request input reaching a command sink, `eval` of a decoder, a call name assembled at runtime, a self-rewriting loader, the campaign's markers. Measured 63/63 on the fleet's malware corpus and 0 CRITICAL on 10,893 clean blueprint files.
- **It is built not to hurt the site.** The engine is loaded inside the cron callback only, inside `try/catch`, never at bootstrap; a broken engine costs one cron run, not a page view. Each run scans for at most 15 seconds and resumes from a cursor, with files modified in the last 72 hours always first, so a fresh drop is seen within the hour while the full sweep continues across runs. Nothing above 256 KB is tokenised (measured 346 bytes of memory per source byte on a token-dense file) and the engine refuses to tokenise without memory headroom, degrading to a regex check instead of a memory fatal. A run that dies on a poison file is noticed by the next run, which skips ahead. Symlinked directories are not descended (Flywheel's root-owned core).
- **Only CRITICAL emails, once per file per day.** HIGH findings are recorded in plugin state for `fw-check-site-v2.sh` to show; there is no fleet manifest inside the plugin to clear dual-use vendor code, and an alert that fires on clean sites is an alert nobody reads.
- **The plugin can verify its own folder.** When a release ships `includes/release-hashes.txt` (sha256 of every PHP file), a PHP file inside `plugins/ds-toolkit/` that is not on the list, or one whose hash differs, is CRITICAL. The workflow step that generates the list needs the `workflow` token scope and follows in its own PR; until then the folder check is skipped, never a false alarm. Tripwire and the engine are exempt from the content scan by exact path, never by content, because a string can be copied by an attacker.
...
## [1.9.130] - 2026-09-16
### Fixed
- **Tripwire now watches the web root, and reads the right one.** The September wave planted a `<hex>wp` web shell at the web root (not wp-content) on rattlers and eysoccer, where it sat through two cleanups and was used 10-14 days later to drop page-shadowing doorway folders (gambling pages to Google, empty 500s to members: 72 failed `/register/` loads in one day). Tripwire ran daily on both sites the whole time and found nothing: no check listed the web root, and the one that tried read `ABSPATH . 'error.php'`, which on Flywheel resolves to the root-owned `/wordpress/` platform directory, not `/www` where the attacker writes. The web root is now `dirname(WP_CONTENT_DIR)` on every host, and it is listed against an **allowlist** of what belongs; anything else is a finding. A `<hex>wp` folder, a folder named after one of the site's own published pages, the Play Store doorway kit (3+ of its 7 files or 3+ bulk sitemaps), and the 8 known attacker Google Search Console tokens are CRITICAL. Attacker names are deliberately not enumerated: the previous web-root rule was `^[0-9a-f]{5,12}$` and missed the entire `<hex>wp` family because `w` and `p` are not hex.
- **"Executable" is judged by extension or header, not extension alone.** Every 600 KB doorway `index.php` was static HTML with no `<?php` tag and PHP ran it anyway; the hidden shells wore `.php5`, `.phtml`, `.php7`, `.png` and `.mov`. A file counts if it has a PHP-family extension or `<?php` in its first 4 KB.
- **Fixed-name shells are hunted where they were found.** `Nx###.php` sat inside every one of 35 fake `starter-*` plugins, `egl.php` and `kir.php` in the fake themes and twice in uploads, plus the self-resurrecting `.sys_cache_log/` nest. A name-only walk (plugins/themes 3 deep, uploads and web root 1-2 deep, 20k entries max) reports them as CRITICAL. Measured 0.3-1 s warm, 2-5 s cold, once a day inside wp-cron.
- **Findings carry a severity, and only CRITICAL and HIGH email.** The subject leads with the tier so the inbox sorts itself. REVIEW findings (an unexpected folder with no executable content, a physical sitemap file, an unknown verification token) are recorded in plugin state only.
- **A legitimate stray folder emails once, not daily.** A site's own bespoke code (`/classes/`) or an old WordPress copy (`/gkb001/`) would otherwise page the team every morning forever. Non-critical web-root findings are remembered in state (`root_seen`) after their first report; CRITICAL never baselines.
- **Design Shop staff adding their own access never emails, from the instant hook too.** Our own cleanup logins on 2026-09-15 arrived via WP-CLI as "unknown actor" and paged the team twice about ourselves. Any `@leagueapps.com` administrator is recorded in the audit log and skipped.
- `DS_Tripwire::scan_root( $dir )` is public and static so the web-root classifier can be pointed at a `.quarantine-*` folder holding the real malware as a test with no fixtures. Verified against 54 quarantined items on two sites before release.
...
- New **Nested Pages Order on the Front End** feature (`features/class-ds-nested-order.php`, on by default) sets `menu_order title` ASC on front-end archive queries for the post types Nested Pages manages. Blog posts and pages are deliberately excluded: a news archive is reverse-chronological by definition, and child-page listings are already handled by `[getsubmenu]`, `[child_pages]` and the Page Cards module. Search, feeds, singular views, the admin, and any query that already declares its own `orderby` are all left alone, so a Beaver Builder loop or another plugin always wins.
- Taxonomy archives needed care: at `pre_get_posts` the generic `taxonomy` and `term` query vars are still EMPTY when a taxonomy has its own query_var (`team-category`), because WP fills those in later. Reading `taxonomy` finds nothing, so the post type is identified from the other direction, by checking each candidate type's taxonomies for a query var the request actually carries.
- **Post Loop: "Menu Order / Nested Pages position" no longer renders backwards.** The Order field defaults to Descending and its labels are date-flavoured ("Descending (newest first)"), which gave an editor no reason to expect that picking Menu Order would reverse the sequence they had just dragged. Nested Pages numbers from 0 at the top, so `menu_order` is now always ascending, matching `[getsubmenu]`, `[child_pages]` and Page Cards, which all hardcode ASC already.
...
- This is why the setting appeared to work in the builder and not on the published page, and why it looked alignment-specific: `--center` is the only alignment modifier that sets `max-width`, so Left and Right never showed the symptom. It affected more than width — the same tie could beat the banner title and subtitle colours against `.ds-banner--has-bg` / `.ds-banner--no-bg`, and the container width added in 1.9.91 against `.ds-hero--banner .ds-hero-wrap`. All are fixed by the same change.

## [1.9.91] - 2026-08-13
### Fixed
- **Hero Banner: Content Width above 1280px was silently clamped (GH #134).** Asking for 1600px rendered 1280px, with nothing in the UI to explain why. The text column sits inside `.ds-hero-wrap`, which carries its own cap — 1280px from the stylesheet, or whatever **Container Width** is set to — and an inner `max-width` can never exceed its container. When the content now asks for more room than its container allows, the container is widened to match, so the value is always honoured. It is only ever widened and only in that case: a Container Width set deliberately narrower than the content is left alone below the cap, and Full-width containers are untouched. The Content Width slider also went to 1280 only, which made values above it unreachable without typing; it now goes to 1920, and both fields' help text explains how they interact.
- Reported as alignment-specific ("ignored when Alignment is Center"), but it is not — measured against the real stylesheet, Left, Center and Right all clamped identically at 1280. Centre is simply where a too-narrow column is most obvious. All three are fixed.

## [1.9.90] - 2026-08-13
### Added
- **Post Loop: Include / Exclude specific posts (GH #132).** Two searchable multi-select pickers under Query → Posts: **Include Specific &lt;Type&gt;** narrows the loop to exactly the entries chosen, and **Exclude Specific &lt;Type&gt;** returns everything the query would normally show minus those. Both list only the currently selected post type, so picking Staff searches staff by name. Number of Posts, Order and Offset all still apply on top — `orderby` is deliberately not forced to `post__in`, so an included set still sorts however the module is set. The pickers are generated per post type and revealed by the Post Type selector, because a Beaver Builder settings form is registered once at `init` and a single field cannot know which type a given module instance will be set to; switching Post Type therefore reveals that type's own picks rather than silently applying another type's ids. They also hide when Source is "Current archive", which ignores this query entirely. Exclude **merges** with Exclude Current Post rather than replacing it — the two write to the same query arg and one clobbering the other would silently drop a filter the editor had set. Applied in both query paths: the shared builder and the Tournament layout's own `get_posts()`, which is exactly the split that let the Taxonomy Filter be ignored there until 1.9.71.

### Fixed
- **Post Loop: "Undefined property: order_by" on layouts saved before that field existed.** The value was read a second time inside the true branch of its own `in_array()` guard, and since the `??` fallback is itself a valid option, an unset property took precisely that branch — so the query got `orderby => null` instead of `date`, plus a PHP notice on every render. Resolved once now. (Same defect as the one fixed in Page Cards in 1.9.82; this was the other copy.)

## [1.9.89] - 2026-08-11
### Added
- **Origin Guard — pre-plugin flood protection.** A new security layer beneath Bot Shield. The toolkit installs and maintains a small must-use plugin (`mu-plugins/ds-origin-guard.php`) that refuses the exact request shapes of a bot flood *before WordPress loads any plugins*, so each attack request costs ~20ms instead of a full render — a 50-100x capacity gain on the attacked endpoints. Bot Shield runs at `plugins_loaded` (after ACF, Beaver Builder, Defender, etc. have all loaded), which the Aug 2026 fleet flood (WPE ticket #8578620: 21k POSTs to `/wp-login.php`, 7.4k to `/wp-json/wp/v2/users/me`) proved is still too expensive when the shared PHP pool is saturated. Origin Guard is the only in-WordPress layer that runs earlier. Three rules, each refusing only traffic no real visitor produces: anonymous REST user-enumeration, credential-stuffing login POSTs that never rendered the login form, and XML-RPC. Logged-in users, authenticated API calls, and normal visitors pass untouched; the XML-RPC rule is auto-skipped on sites running Jetpack or the WP mobile app (detected at install time). ON fleet-wide by default; turning the toggle off deletes the mu-plugin. The toolkit owns the file — it refreshes it when the payload version changes and removes it on disable. Per-rule toggles for the login and XML-RPC rules live under the Origin Guard card in **Security & Access**.

## [1.9.88] - 2026-08-11
### Added
- **Theme Setting: Image Overlay on Base Page Background, Base Page Content Background and Page Banner.** A colour tint over the background image, for holding text legible over a busy photo. Use a colour with transparency — the picker keeps its alpha channel, and a solid colour simply hides the image. It is stacked as a flat gradient inside the same `background-image` shorthand, so none of the three surfaces needs an extra element or pseudo-element; they are all painted on markup the base Themer template owns, and adding DOM there would ripple through every build. With no image set the overlay is skipped entirely (a tint over a plain colour is just a different colour, which is what Background Color is for), so a site that sets one without an image sees no change.
- **Theme Setting: Position and Attachment on the Page Banner.** The other two surfaces already had both; the banner's position was hardcoded to `center` and it had no attachment control at all. The new Position setting defaults to `center center`, which is the value that was hardcoded, so no existing banner moves.

### Fixed
- **Theme Setting: "Attachment: Fixed" (the parallax effect) is no longer broken on phones.** iOS Safari does not support `background-attachment: fixed` — it falls back to scroll but keeps sizing the image against the viewport, so a `cover` background renders zoomed and cropped. Since most partner traffic is iPhone, choosing Fixed was actively worse than Scroll on the devices that matter most. Whenever Fixed is selected, coarse-pointer devices are now switched back to `scroll`, as is anyone who has asked for reduced motion — a background sliding under the content is exactly the movement that request covers. Applies to all three surfaces, and the setting's help text now says what Fixed does and that it self-disables in those two cases.

## [1.9.87] - 2026-08-11
### Fixed
- **Post Loop: the Tournament card background could not be changed (GH #128).** The cards stayed white whatever was picked. There was no setting for the card at all: `.ds-tourn--event .ds-tourn-card` hardcoded `background: var(--fl-global-white)` in the stylesheet, and the existing **Details Background** control styles `.ds-tourn-body` — the inner panel that overlaps the image on the Overlap layout, which is a different element. A new **Card Background** control under Style → Tournament Cards colours the card itself, with alpha support and a live builder preview. Blank emits nothing, so the built-in white remains the default and no existing build changes. The rule is scoped to `.ds-tourn-card` alone, so the filter/search bar, the grid and the surrounding section are untouched, and it carries three classes because the hardcoded rule it has to beat is itself two — matching on specificity and relying on source order would have been fragile. The **Details Background** control keeps its behaviour and now says in its help text which element it actually paints, since the two were easy to confuse.
...
### Fixed
- **Carousel: a wide caption image no longer pushes the caption text out of the pill.** Found while testing the above with a real-world upload: partners add whatever image they have, and a wide one resolved to several hundred pixels at the authored height, squeezing the label down to one character per line. The image is now capped and letterboxes past that width instead of claiming the space. The cap needed **two** classes to take effect — Beaver Builder ships `.fl-module img{max-width:100%}` at specificity (0,1,1), which silently outranks a plugin's single class (0,1,0), the same trap that once painted our buttons white-on-white.
