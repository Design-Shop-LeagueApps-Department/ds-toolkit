...
- **LeagueApps Table: a Beaver Builder module for schedules, fees, rosters and standings. Not released yet: on branch `feat/table-module`, being tested on ds-launchpad-7** (Alipio, 2026-09-26: "an easy way to create table using beaver UI they can also upload CSV. make sure its ajax and well sync"). Sample: the Tryout Schedule section on ds-launchpad-7's Tryouts page.
  ...
  - **CSV and sync, all over AJAX.** Importing a CSV into the editor reads the file and keeps nothing on the server, so a roster CSV never sits in the Media Library at a public URL with columns the table left out; the file is stored only when "Keep synced to this file" is chosen. "Upload new version" overwrites that file in place (same attachment, same URL), so every table synced to it updates without republishing the page. A Google Sheets share link is turned into its CSV export link. Fetched copies are cached for the chosen interval, the last copy that loaded is kept (no expiry) and served when the sheet cannot be reached, a lock stops two visitors fetching at once, a failure backs off for a minute, and "Refresh now" fetches a link at most once every 20 seconds. Parsing handles a byte-order mark, Windows-1252 exports from Excel, comma, semicolon, tab and pipe delimiters, and quoted commas and line breaks, and caps a file at 5,000 rows, 50 columns, 2 MB and 30,000 cells in all.
  - **On the page.** Sortable columns that sort numbers, prices, dates and times as such (first to last, last to first, then back to the typed order), optional search with a no-match message, a row count, pagination, and on phones either stacked cards (every value labelled with its column, plus a sort dropdown) or a table that scrolls sideways. Web addresses and email addresses in cells become links, and cell text is escaped.
  - **Style.** Five presets (Striped, Bordered, Minimal, Framed, Brand) act as a base layer that any Style field overrides; the header background is From the preset, None or Pick a colour; colour fields take global colours; the header and rows have their own typography. The sort headings and pager are reset against the theme's global Button style, so they never render as filled buttons. The module requests the bold (700) face of the fonts it uses, because Beaver Builder loads only the weights its settings name and the browser otherwise fakes the bold.
  - **Images, links and buttons in cells** (Alipio, 2026-09-27: "how can we add image or url on the cell?"). Each column has a Type in its menu: Text, Image, Link or Button. An Image cell holds a Media Library image (a "Choose" button in the editor, with a thumbnail) or an image address (a Google Drive share link is turned into its public thumbnail); `image | address` makes it clickable. A Link or Button cell holds `Label | address` or `[Label](address)`, and a lone address takes the column heading as its label; Buttons use the site Button style (small by default). Any Text cell also takes inline `[label](address)` links. Addresses are limited to http(s), mailto, tel and site paths (`/register/`), so `javascript:` and `data:` never become links or images. Images lazy-load with fixed dimensions, keep the Media Library alt text or fall back to the row's first cell, and can be sized, rounded or circled, and fitted (logos) or cropped (photos) in Style > Images & buttons. Image columns are not sortable; Link and Button columns sort and search by their label. On phones the image leads each card and a button spans it.
  - **Security and stability audit (2026-09-27).** Fixed before release:
    - **Pager labels could run script** (stored XSS): the Previous / Next text was put into the page as HTML, and an entity-encoded tag passes Beaver Builder's own filter for users without `unfiltered_html`. The labels are now escaped. Proven in a browser: the old script ran the payload, the new one shows it as text.
    - **Colour fields could inject CSS** (`#fff;}body{display:none`). The shared `DS_Module_UI::color()` now accepts only real colours (hex, rgb/hsl, `var(--x)` with a fallback, keywords) and returns nothing otherwise, which covers every LeagueApps module that uses it. Every colour value stored on ds-launchpad-7 comes out unchanged.
    - **Page weight:** every row is in the HTML (paging and search run in the browser), so a 5,000 x 50 table was a 30 MB page. Tables are now capped at 30,000 cells (3,000 rows of 10 columns); the editor says so when a table goes over, and a CSV import warns. Rows no longer repeat their text in `data-search` and text sort keys (the browser reads the cells), so the worst case is 3.3 MB.
    - **Link sync behind a page cache:** a cached page never runs PHP, so "Check every 5 minutes" did nothing for visitors. Publishing now records a page's synced links; a WP-Cron job fetches each one on its own schedule and clears that page's cache (WP Engine `WpeCommon`, plus a `ds_table_purge_post` hook) once per data change, never on a failed fetch. "Upload new version" clears every page showing that file.
    - **Sync correctness:** each table's own interval is honoured (two tables sharing a sheet no longer share one expiry), the last good copy never expires (a failed first fetch after a week no longer page-caches an empty table), a failed fetch now reports its error in the builder (it was dropped whenever an old copy existed), and cached rows are stored compressed (a 2 MB sheet was 3.6 MB, over a memcached object cache's 1 MB item limit).
    - **Large files:** a Media Library CSV is read only up to the 2 MB limit (any size was read into memory on every render); a cut is made at a line break, so accents are no longer garbled and no half row is imported; a header plus 5,000 rows keeps all 5,000.
    - **Cells:** an email inside a web address (`?ref=coach@club.org`) broke the link's markup; `Label | image` now shows the image with the label as alt text; `/u/0/` Google Sheets links convert.
    - **Endpoints:** the table AJAX actions also require Beaver Builder access, the link fetch is limited to 30 a minute per user, and it reads the link exactly as the module stores it (so "Refresh now" warms the cache visitors read).
  - **Storage.** The table is one hidden setting holding `dst1:` plus base64 JSON, because Beaver Builder turns any setting that looks like JSON into objects on save.
  - **Tests.** `tests/table-data-test.php` (75 assertions: storage, CSV parsing, column types and sort keys, Google links, escaping, link / button / image cells and unsafe addresses, the cell budget, cut-off files, compressed caching, colour injection, and link freshness / last good copy / failure back-off with HTTP faked), `tests/table-sync-test.php` (10: what publishing records, and that the sync job clears a page's cache once per change and never on a failure) and `tests/table-ajax-test.php` (13: import keeps no file, and who may read, store or replace a CSV; the handler before the read-only import fails 4 of them), all run with `wp eval-file`. End to end in the real builder on ds-launchpad-7: 33 builder checks, 20 front-end checks and 30 for image / link / button columns (including picking an image from the Media Library).
